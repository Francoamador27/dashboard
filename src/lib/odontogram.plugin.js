...
// Opcional (algunos plugins esperan $ global)
if (!window.$) window.$ = $;
if (!window.jQuery) window.jQuery = $;


// CONSTANTA untuk MODE Odontogram
export var ODONTOGRAM_MODE_HAPUS = 100; // HAPUS
export var ODONTOGRAM_MODE_DEFAULT = 0; // Do Nothing
export var ODONTOGRAM_MODE_COF = 2; // Hijau Diarsir = TAMBALAN COMPOSITE
export var ODONTOGRAM_MODE_AMF = 1; // Hitam = TAMBALAN AMALGAM
export var ODONTOGRAM_MODE_FIS = 3; // UNGU = pit dan fissure sealant
export var ODONTOGRAM_MODE_NVT = 4; // SEGITIGA DIBAWAH (seperti Akar) = gigi non-vital
export var ODONTOGRAM_MODE_RCT = 5; // SEGITIGA DIBAWAH (seperti Akar) filled = Perawatan Saluran Akar
export var ODONTOGRAM_MODE_NON = 6; // gigi tidak ada, tidak diketahui ada atau tidak ada. (non)
export var ODONTOGRAM_MODE_UNE = 7; // Un-Erupted (une)
export var ODONTOGRAM_MODE_PRE = 8; // Partial-Erupt (pre)
export var ODONTOGRAM_MODE_ANO = 9; // Anomali (ano), Pegshaped, micro, fusi, etc
export var ODONTOGRAM_MODE_CARIES = 10; // Caries = Tambalan sementara (car)
export var ODONTOGRAM_MODE_CFR = 11; // fracture (cfr) (Tanda '#' di tengah" gigi)
export var ODONTOGRAM_MODE_FMC = 12; // Full metal crown pada gigi vital (fmc)
export var ODONTOGRAM_MODE_POC = 13; // Porcelain crown pada gigi vital (poc)
export var ODONTOGRAM_MODE_RRX = 14; // Sisa Akar (rrx)
export var ODONTOGRAM_MODE_MIS = 15; // Gigi hilang (mis)
export var ODONTOGRAM_MODE_IPX = 16; // Implant + Porcelain crown (ipx - poc)
export var ODONTOGRAM_MODE_FRM_ACR = 17; // Partial Denture/ Full Denture
export var ODONTOGRAM_MODE_BRIDGE = 18; // BRIDGE
export var ODONTOGRAM_MODE_ARROW_TOP_LEFT = 19; // TOP-LEFT ARROW
export var ODONTOGRAM_MODE_ARROW_TOP_RIGHT = 20; // TOP-RIGHT ARROW
export var ODONTOGRAM_MODE_ARROW_TOP_TURN_LEFT = 21; // TOP-TURN-LEFT ARROW
export var ODONTOGRAM_MODE_ARROW_TOP_TURN_RIGHT = 22; // TOP-TURN-RIGHT ARROW
export var ODONTOGRAM_MODE_ARROW_BOTTOM_LEFT = 23; // BOTTOM-LEFT ARROW
export var ODONTOGRAM_MODE_ARROW_BOTTOM_RIGHT = 24; // BOTTOM-RIGHT ARROW
export var ODONTOGRAM_MODE_ARROW_BOTTOM_TURN_LEFT = 25; // BOTTOM-TURN-LEFT ARROW
export var ODONTOGRAM_MODE_ARROW_BOTTOM_TURN_RIGHT = 26; // BOTTOM-TURN-RIGHT ARROW
...
(function ($) {
    ...
    function AMF(vertices, options) {
        this.name = 'AMF';
        this.vertices = vertices;
        this.options = $.extend({ fillStyle: 'blue' }, options);
        return this;
    }
    ...
    function FIS(vertices, options) {
        this.name = 'FIS';
        this.vertices = vertices;
        this.options = $.extend({ fillStyle: 'red' }, options);
        return this;
    }
    ...
    NVT.prototype.render = function (ctx) {
        ...
        var radius = 20; // Radio para esquinas redondeadas

        ctx.strokeStyle = "blue"; // Cambiar a azul
        ctx.lineWidth = 6;

        ctx.beginPath();
        ctx.moveTo(x1 + radius, y1); // Inicia con un margen del radio
        ctx.arcTo(x2, y1, x2, y2, radius); // Esquina superior derecha
        ctx.arcTo(x2, y2, x1, y2, radius); // Esquina inferior derecha
        ctx.arcTo(x1, y2, x1, y1, radius); // Esquina inferior izquierda
        ctx.arcTo(x1, y1, x2, y1, radius); // Esquina superior izquierda
        ctx.closePath();
        ctx.stroke();

        // Dibujar líneas verticales internas
        ctx.lineWidth = 1;
    ...
    function ANO(vertices, options) {
        this.name = 'ANO';
        this.vertices = vertices;
        this.options = $.extend({ fillStyle: 'red', fontsize: 12 }, options);
        return this;
    }
    ANO.prototype.render = function (ctx) {
        ...
        // Establecer el color rojo para las líneas
        ctx.strokeStyle = "red";
        ctx.lineWidth = 4;
    ...
    function CARIES(vertices, options) {
        this.name = 'CARIES';
        this.vertices = vertices;
        this.options = $.extend({ strokeStyle: 'blue' }, options);
        return this;
    }
    ...
    function POC(vertices, options) {
        this.name = 'POC';
        this.vertices = vertices;
        this.options = $.extend({ strokeStyle: 'red' }, options);
        return this;
    }
    POC.prototype.render = function (ctx) {
        ...
        ctx.strokeStyle = this.options.strokeStyle; // Mantiene el color original
        ctx.lineWidth = 6;

        // Dibujar el rectángulo con esquinas redondeadas
        ctx.beginPath();
        ctx.moveTo(x1 + radius, y1); // Comienza con un margen del radio
        ctx.arcTo(x2, y1, x2, y2, radius); // Esquina superior derecha
        ctx.arcTo(x2, y2, x1, y2, radius); // Esquina inferior derecha
        ctx.arcTo(x1, y2, x1, y1, radius); // Esquina inferior izquierda
        ctx.arcTo(x1, y1, x2, y1, radius); // Esquina superior izquierda
        ...
        // Dibujar las líneas verticales internas
        ctx.lineWidth = 1;
        for (var xpos = x1; xpos < x2; xpos += ((x2 - x1) / 15)) {
            xpos = Math.min(xpos, x2);
    ...
    function BRIDGE(startVert, endVert, options) {
        ...
        this.fromTooth = options.fromTooth || null; // Número del diente de inicio
        this.toTooth = options.toTooth || null;     // Número del diente de fin
        return this;
    }

    BRIDGE.prototype.render = function (ctx) {
        // Dibujar el puente (código existente)
        var vert0, vert1;
        if (this.startVert) {
            vert0 = {
        ...
        // Mostrar información de los dientes conectados
        if (this.fromTooth && this.toTooth) {
            ctx.fillStyle = '#000';
            ctx.font = '12px Arial';
            ctx.textAlign = 'center';
            ctx.fillText(
                `Puente: ${this.fromTooth} a ${this.toTooth}`,
                (vert0.cx + vert1.cx) / 2,
                (vert0.y1 + vert1.y1) / 2 - 20
            );
        }
    };
    // Class HAPUS
    ...
    Odontogram.prototype._centerTeeth = function (ctx, numbers, bigBoxSize, smallBoxSize, xpos, ypos) {
        // Small Box (corregido para ser cuadrado)
        ctx.beginPath();
        ctx.lineWidth = "2";
        ctx.strokeStyle = "#555";
        ctx.rect(
            xpos + smallBoxSize / 2,    // X centrado
            ypos + smallBoxSize / 2,    // Y centrado
            smallBoxSize, 
            smallBoxSize               // Alto = Ancho (cuadrado)
        );
        ctx.stroke();
    
        // Líneas (ajustadas como en _sideTeeth)
        //// Top Left
        ctx.beginPath();
        ctx.moveTo(xpos, ypos);
        ctx.lineTo(xpos + smallBoxSize / 2, ypos + smallBoxSize / 2);
        ...
        // Coordenadas corregidas (sin ajustes +3/-3)
        const x1 = xpos;
        const y1 = ypos;
        const x2 = xpos + bigBoxSize;
        const y2 = ypos + bigBoxSize;
        const cx = xpos + bigBoxSize / 2;  // Centrado correcto
        const cy = ypos + bigBoxSize / 2;  // Centrado correcto
        const key = x1 + ':' + y1 + ';' + x2 + ':' + y2 + ';' + cx + ':' + cy;
        ...
        this.teeth[key] = {
            ...
            cx: cx,
            cy: cy,
            // Coordenadas corregidas (igual que en _sideTeeth)
            top: {
                tl: { x: xpos, y: ypos },
                tr: { x: xpos + bigBoxSize, y: ypos },
                br: { x: xpos + bigBoxSize - smallBoxSize / 2, y: ypos + smallBoxSize / 2 },
        ...
        switch (mode) {
            ...
            case ODONTOGRAM_MODE_BRIDGE:
                newGeometry = new BRIDGE(geometry[0], geometry[1], {
                    strokeStyle: '#555',
                    fromTooth: geometry.fromTooth, // Asegúrate de pasar fromTooth
                    toTooth: geometry.toTooth     // Asegúrate de pasar toTooth
                });
                break;
    ...
    function _joinShapeTeeth(geoms1, geoms2) {
        ...
        // Agregar todas las propiedades de geoms1
        if (geoms1 && geoms1.length > 0) {
            for (var x = 0; x < geoms1.length; x++) {
                geometry.push(geoms1[x]);
            }
        }

        // Agregar todas las propiedades de geoms2
        if (geoms2 && geoms2.length > 0) {
            for (var y = 0; y < geoms2.length; y++) {
                geometry.push(geoms2[y]);
            }
        }
            ...
            switch (instance.mode) {
                ...
                case ODONTOGRAM_MODE_BRIDGE:
                    if (isRectIntersect(coord, { x1: mouse.x, y1: mouse.y, x2: mouse.x, y2: mouse.y })) {
                        tempGeoms[keyCoord] = [];
                        if (instance.active_geometry) {
                            // Obtener los números de los dientes conectados
                            var fromTooth = instance.active_geometry.startVert ? instance.active_geometry.startVert.num : null;
                            var toTooth = teeth.num;
                            ...
                            instance.active_geometry = convertGeom([
                                ...
                                fromTooth: fromTooth, // Pasar fromTooth
                                toTooth: toTooth      // Pasar toTooth
                            });
