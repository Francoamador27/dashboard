...
export function useActiveAssets() {
  return useQuery({
    ...
    queryFn: () =>
      api.get('/assets', { params: { status: 'pending,processing', per_page: 100 } }).then((r) => r.data),
...
export function useRecentAssets() {
  return useQuery({
    ...
    queryFn: () => api.get('/assets', { params: { per_page: 32 } }).then((r) => r.data),
...
export function useMoverColeccion() {
  ...
  return useMutation({
    mutationFn: ({ id, coleccion_id }) =>
      api.post(`/assets/${id}/mover-coleccion`, { coleccion_id }).then((r) => r.data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['assets'] }),
  });
}
...
export function useSubirAssets(clienteId) {
  ...
  return useMutation({
    mutationFn: (formData) =>
      api.post(`/clientes/${clienteId}/assets/upload`, formData).then((r) => r.data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['assets'] }),
  });
}
