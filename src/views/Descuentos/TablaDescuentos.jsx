...
const TablaDescuentos = () => {
  ...
  const obtenerCupones = async () => {
    try {
      const res = await clienteAxios.get('/api/cart-discounts', {
        headers: {
  ...
  const eliminarCupon = async (id) => {
    ...
    try {
      await clienteAxios.delete(`/api/cart-discounts/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      <table className="min-w-full text-sm text-gray-800 bg-white">
        <thead className="bg-gray-100 text-xs text-gray-600 ">
          <tr>
            ...
            <th className="px-4 py-3 text-left">Límite Uso</th>
        ...
        <tbody>
          ...
          {cupones.map((cupon) => (
            <tr key={cupon.id} className="hover:bg-gray-50 border-t border-gray-200 transition">
              ...
              <td className="px-4 py-3 font-semibold">{cupon.code}</td>
              <td className="px-4 py-3 capitalize">{cupon.type}</td>
              <td className="px-4 py-3">{cupon.discount_value}</td>
              <td className="px-4 py-3">
                {cupon.condition_type === 'amount'
                  ? `Mín. $${cupon.min_value}`
                  : `Mín. ${cupon.min_value} productos`}
              </td>
